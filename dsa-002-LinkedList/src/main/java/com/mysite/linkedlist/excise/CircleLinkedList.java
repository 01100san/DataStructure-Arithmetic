package com.mysite.linkedlist.excise;
...
public class CircleLinkedList<E> extends AbstractList<E> implements Serializable, List<E> {
    ...
    /**
     * 只在当前类中使用，添加私有的remove方法，按照节点删除 => 删除当前current节点
     * @param oldNode 删除的节点
     * @return 删除的节点元素
     */
    private E remove(Node<E> oldNode){
        if (size == 1){
            ...
            if (oldNode == first) {  //index == 0     oldNode == first
    ...
    public E remove(){
        ...
        if (size == 0){     //size = 0, 说明删除的是current本身，将current置为null
            current = null;
        }else {
            current = next;
        }
